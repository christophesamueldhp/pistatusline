...
function getSettingsPaths(): { configDir: string; settingsPath: string; backupPath: string } {
    const configDir = path.join(MOCK_HOME_DIR, '.pi', 'agent', 'pistatusline');
    return {
        configDir,
...
describe('config utilities', () => {
    ...
    beforeEach(() => {
        ...
        // pistatusline: first-run seeding reads ~/.config/ccstatusline, so HOME must be the mock.
    ...
    it('seeds from an existing ccstatusline config on first run', async () => {
        const { settingsPath } = getSettingsPaths();
        const ccstatuslineDir = path.join(MOCK_HOME_DIR, '.config', 'ccstatusline');
        fs.mkdirSync(ccstatuslineDir, { recursive: true });
        fs.writeFileSync(path.join(ccstatuslineDir, 'settings.json'), JSON.stringify({
            version: CURRENT_VERSION,
            lines: [[{ id: 'm', type: 'model', color: 'brightWhite' }]],
            gitCacheTtlSeconds: 7
        }));

        const settings = await loadSettings();

        expect(settings.lines[0]).toEqual([{ id: 'm', type: 'model', color: 'brightWhite' }]);
        expect(settings.gitCacheTtlSeconds).toBe(7);

...
describe('usage window helpers', () => {
    ...
    // pistatusline: no fallback to Claude Code's cached 5-hour block.
    it('never falls back to Claude Code\'s cached block when usage reset data is missing', () => {
        mockGetCachedBlockMetrics.mockReturnValue({
            startTime: new Date('2026-03-02T15:00:00.000Z'),
            lastActivity: new Date('2026-03-02T18:00:00.000Z')
        });

        const window = resolveUsageWindowWithFallback({}, undefined, Date.parse('2026-03-02T18:00:00.000Z'));

        expect(window).toBeNull();
        expect(mockGetCachedBlockMetrics.mock.calls.length).toBe(0);
    });

    it('still uses block metrics handed to it explicitly', () => {
        const nowMs = Date.parse('2026-03-02T18:00:00.000Z');
        const window = resolveUsageWindowWithFallback({}, {
            startTime: new Date('2026-03-02T15:00:00.000Z'),
            lastActivity: new Date('2026-03-02T18:00:00.000Z')
        }, nowMs);

...
// pistatusline: pi has no transcript file; its session entries arrive as in-memory records.
async function* iterateTranscriptRecords(transcriptPath: string, records?: readonly unknown[]): AsyncGenerator<unknown> {
    if (records) {
        yield* records;
        return;
    }
...
async function scanTranscript(transcriptPath: string, options: TranscriptScanOptions, records?: readonly unknown[]): Promise<TranscriptScanResult> {
    ...
    if (!records && !fs.existsSync(transcriptPath)) {
        return emptyResult;
    }
    ...
    try {
        for await (const record of iterateTranscriptRecords(transcriptPath, records)) {
            const data = record as TranscriptLine | null;
...
export async function getTranscriptAnalysis(
    ...
    return toTranscriptAnalysis(await scanTranscript(transcriptPath, {
        ...options,
        includeTokenMetrics: true
    }));
}

// pistatusline: same analysis over in-memory transcript records (no subagent files to follow).
export async function getTranscriptAnalysisFromRecords(
    records: readonly unknown[],
    options: TranscriptAnalysisOptions = {}
): Promise<TranscriptAnalysis> {
    return toTranscriptAnalysis(await scanTranscript('', {
        ...options,
...
function toTranscriptAnalysis(result: TranscriptScanResult): TranscriptAnalysis {
    return {
